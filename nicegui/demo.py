...
class ChartLinker:
    ...
    def highlight_corresponding_element(self, element_name: str, target_chart: str):
        ...
        if target_chart == "graph" and config["graph_index"] is not None:
            self.graph.options["series"]["nodes"][config["graph_index"]]["itemStyle"][
                "color"
            ] = Colors.HIGHLIGHT
            self.graph.update()
        elif target_chart == "gantt":
            self.gantt.options["series"][config["gantt_index"]]["itemStyle"][
                "color"
            ] = Colors.HIGHLIGHT
            self.gantt.update()
    ...
    def reset_colors(self):
        ...
        for name, config in SERIES_CONFIG.items():
            if config["graph_index"] is not None:
                self.graph.options["series"]["nodes"][config["graph_index"]][
                    "itemStyle"
                ]["color"] = config["color"]
